...
<body>
  ...
  <main class="container py-4">
    ...
    <div class="progress mb-4" style="height: 8px; border-radius: 10px;">
      <div id="progressBar" class="progress-bar bg-primary" role="progressbar" style="width: 0%;" aria-valuenow="0"
    ...
    <section id="step-1">
      ...
      <div class="col">
        <div id="template-{{ $template->id }}" class="card h-100 shadow-sm border cursor-pointer"
        ...
        <img src="{{ asset('storage/' . $template->file_path) }}" class="card-img-top preview-img"
          style="height: 150px; object-fit: cover;" alt="Template {{ $template->id }}">
        </div>
      </div>
    @endforeach
      </div>
    ...
    <section id="step-2" class="hidden">
      ...
      <div class="d-flex gap-4 align-items-start flex-wrap">
        ...
        <div class="border rounded p-4 bg-light text-center" style="flex: 1;">
          <img id="preview-image" src="#" alt="Preview Template" class="preview-img mx-auto img-fluid"
        ...
        <div id="template-info" style="flex: 1;">
          <h3>Template Info</h3>
          <p class="mt-4"><strong>Nama :</strong><span id="template-name"> {{ $template->user->name ?? 'Tidak Diketahui' }}</span></p>
          <p class="mt-4"><strong>Type : </strong> <span id="template-type">{{ $template->type ?? '-' }}</span></p>
          <p class="mt-4"><strong>Size : </strong> <span id="template-size"></span></p>
          <p class="mt-4"><strong>Created :</strong><span id="template-date"> {{  $template->date ? \Carbon\Carbon::parse($template->date)->format('d-m-Y') : '-' }} </span></p>
          <p class="mt-4"><strong><label for="description">Description:</label><br></strong><span id="template-description">{{ $template->description ?? '-' }}</span></p>
        ...
        <div class="col-md-7 mb-3">
          <div class="position-relative border p-4 bg-light rounded text-center">
            ...
            <div class="position-relative" style="width: 100%; max-height: 400px; overflow: hidden;">
              <img id="cert-preview-image" src="#" class="img-fluid border rounded shadow-sm w-100"
              ...
              <div id="preview-participant-name" class="draggable-text resizable-text"
                style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: black; font-weight: bold; font-size: 24px; text-shadow: 1px 1px 2px #fff;">
                <span class="text-content">Nama Peserta</span>
                <div class="resize-handle"></div>
              </div>

              <div id="preview-event-name" class="draggable-text resizable-text"
                style="position: absolute; bottom: 30%; left: 50%; transform: translateX(-50%); color: black; font-size: 30px; text-shadow: 1px 1px 2px #fff;">
    ...
    <div class="step-buttons d-flex justify-content-center mt-4">
      ...
      <button id="back-btn" class="btn border border-primary text-[#1533B5] rounded mx-3"
